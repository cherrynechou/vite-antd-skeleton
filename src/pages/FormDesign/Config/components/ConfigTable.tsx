...
import {queryGroups} from "@/api/auth/ConfigGroupController";
import CreateOrEditConfig from "./CreateOrEditConfig";

type TableRowSelection<T extends object = object> = TableProps<T>['rowSelection'];

export interface IConfigGroupTableProps {
    setConfigGroupId?: (id:  number[] ) => void;
}
...
const ConfigTable:FC<IConfigGroupTableProps> = (props: any) =>{
    ...
    useEffect(() => {
        if(selectedRowKeys){
            setConfigGroupId(selectedRowKeys)
        }
    }, [selectedRowKeys]);
    ...
    const columns: ProColumns<TableListItem>[] = [
        {
            ...
            title: (
                t('pages.searchTable.config.group.name')
            ),
            width: 80,
            align: 'center',
            dataIndex: 'name'
            ...
            title: (
                t('pages.searchTable.config.group.key')
            ),
            width: 80,
            align: 'center',
            dataIndex: 'key',
