export default {
    'pages.404.subTitle': '抱歉，您访问的页面不存在。',
    'pages.404.buttonText': '返回首页',
    'pages.403.subTitle': '无权限',
    'pages.403.buttonText': '返回首页',
    'pages.searchTable.username': '用户名',
    'pages.searchTable.name': '名称',
    'pages.searchTable.telephone': '电话',
    'pages.searchTable.status': '状态',
    'pages.searchTable.new': '添加',
    'pages.searchTable.edit': '编辑',
    'pages.searchTable.role': '角色',
    'pages.searchTable.permission': '权限',
    'pages.searchTable.disabled': '是否禁用',
    'pages.searchTable.loginCount': '登录次数',
    'pages.searchTable.router': '路由',
    'pages.searchTable.slug': '标识',
    'pages.searchTable.icon': '图标',
    'pages.searchTable.code': '编码',
    'pages.searchTable.dict.name': '字典名称',
    'pages.searchTable.dict.code': '代码',
    'pages.searchTable.dict.label': '标签',
    'pages.searchTable.dict.data': '配置数据',
    'pages.searchTable.dict.data.label': '字典标签',
    'pages.searchTable.dict.data.value': '字典值',
    'pages.searchTable.dict.data.isDefault': '默认值',
    'pages.searchTable.dict.data.sort': '排序',
    'pages.searchTable.config.group.name': '配置分组',
    'pages.searchTable.config.group.key': '配置组键名',
    'pages.searchTable.config.data.label': '配置标签',
    'pages.searchTable.config.data.type': '配置键类型',
    'pages.searchTable.config.data.key': '配置键名',
    'pages.searchTable.config.data.value': '配置键值',
    'pages.searchTable.config.option.select.edit': '选项编辑',
    'pages.searchTable.delete': '删除',
    'pages.searchTable.action': '操作',
    'pages.searchTable.locale': '国际化',
    'pages.searchTable.createdBy': '登录人',
    'pages.searchTable.path': '地址',
    'pages.searchTable.httpMethod': 'Http方法',
    'pages.searchTable.ip': 'Ip',
    'pages.searchTable.account': '帐号',
    'pages.searchTable.loginIp': '登录Ip',
    'pages.searchTable.browser': '浏览器',
    'pages.searchTable.platform': '平台',
    'pages.searchTable.sort': '排序',
    'pages.searchTable.ok': '确定',
    'pages.searchTable.cancel': '取消',
    'pages.searchTable.okConfirm': '确定操作?',
    'pages.searchTable.createdAt': '添加时间',
    'pages.searchTable.updatedAt': '更新时间',
    'pages.searchTable.deletedAt': '更新时间',
    'pages.tree.dict': '字典',
}